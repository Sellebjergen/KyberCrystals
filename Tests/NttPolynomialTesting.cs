...
public class NttPolynomialTesting
{
    ...
    public void Ntt_CanBeRevertedBy_InvNttAndFromMontgomery()
    {
        ...
        Assert.True(u[1] == 1);
        Assert.True(u[2] == 1);
        for (var i = 3; i < p.GetPaddedCoefficients(256).Count; i++) Assert.True(p.GetCoefficient(i) == 0);
    }
    ...
    public void Ntt_CanBeRevertedBy_InvNttAndFromMontgomery_only2Pols()
    {
        ...
        for (var i = 0; i < p.GetCoefficients().Count; i++) Assert.True(u[i] == 2);
        for (var i = p.GetCoefficients().Count; i < 256; i++) Assert.True(u[i] == 0);
    }
